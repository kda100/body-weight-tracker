...
class BodyWeightTrackerProvider with ChangeNotifier {
  ...
  final List<WeightRecord> _targetWeightRecords =
      []; //stores target weight records for given range.
  ...
  bool _refreshDataFlag =
      false; //used to control the re-fetching of weight records of body weight tracker.
  ...
  int get weightPointsLen => _weightRecords.length;

  int get targetPointsLen => _targetWeightRecords.length;
  ...
  ///triggers re-fetching of weight records data.
  ...
  ///changes max and min weights when a new weight record hsa been added or read from firebase.
  ...
  ///called when body weight tracker needs to fetch new weight record data for a new date range
  ...
  ///fetches and sets target data and is only called at the start of the application.
  ...
  ///fetches and sets weight records their ids at start of application or user changes date range.
