import 'package:body_weight_tracker/services/firebase_services.dart';
...
class BodyWeightTrackerProvider with ChangeNotifier {
  ...
  double get maxWeightChartLimit =>
      _weightRecords.length > 0 || _targetWeightRecords.length > 0
          ? _maxWeight * 1.2
          : 100;

  double get minWeightChartLimit =>
      _weightRecords.length > 0 || _targetWeightRecords.length > 0
          ? _minWeight * 0.8
          : 0;
  ...
  Future<UpdateStatus> addAndDeleteWeightRecord(
    ...
    if (weightRecordWithId != null) {
      if (!dateTime.isBefore(dateTimeRange.start) &&
        ...
        _weightRecords[index] = weightRecordWithId; //replaces weight record
        _adjustMaxAndMinWeightsWithRecords();
        _toggleRefreshChartFlag();
      }
      return UpdateStatus.SUCCESS;
